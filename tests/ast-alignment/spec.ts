import fs from 'fs';
import { fixturesToTest, fixturesToTestEspree } from './fixtures-to-test';
...
describe('Espree', () => {
  fixturesToTestEspree.forEach(fixture => {
    const filename = fixture.filename;
    const source = fs.readFileSync(filename, 'utf8').replace(/\r\n/g, '\n');
     ...
     */
    if (typeScriptESTreeResult.parseError) {
      it(`TEST FAIL [TS-ESTREE ERRORED, BUT ESPREE DID NOT] - ${filename}`, () => {
