...
export abstract class AbstractConverter {
   ...
   */
  protected convertType(node?: ts.Node, parent?: ts.Node): es.BaseNode | null {
  ...
  protected convert(
    ...
    let result: es.BaseNode | null = null;
  ...
  createNode<T extends es.BaseNode>(
    node: ts.Node,
    data: T
  ): T & { range: [number, number]; loc: es.SourceLocation } {
    const result: T = data;
    if (!result.range) {
  ...
  createSimpleNode(node: ts.Node, name: string): es.BaseNode {
  ...
  convertBodyExpressionsToDirectives(
    node: ts.Node,
    body: (es.ExpressionStatement | null)[]
  ) {
   ...
   */
  convertTypeAnnotation(child: ts.TypeNode): es.TSTypeAnnotation {
   ...
   */
  convertParameters(parameters: ts.NodeArray<ts.Node>): es.BaseNode[] {
    if (!parameters || !parameters.length) {
      return [];
    }
    return parameters.map(param => {
      const convertedParam = this.convert(param);
